...
const StoryCircleListItem = ({
  ...
  return (
    <View>
      ...
      > */}
      <TouchableOpacity onPress={() => _handleItemPress(item)} style={styles.itemContainer}>
        <View>
          {item.user_image ? <Image source={{ uri: item.user_image }} style={isCommunityStory ? styles.communityAvatar : styles.avatar} /> :
            <SvgXml
          ...
          <SvgXml
            ...
            xml={storyRing(item?.stories?.length > 0 ? storyRingColor[0] : '#EBECEF', item?.stories?.length > 0 ? (storyRingColor.length > 1 ? storyRingColor[1] : storyRingColor[0]) : '#EBECEF')}
          />
        ...
        <View style={styles.textRow}>
          ...
          <Text numberOfLines={1} ellipsizeMode='tail' style={styles.itemText}>
            {isCommunityStory ? 'Story' : item.user_name}</Text>
        </View>
